...
void write_launcher(const std::string& bin_rel) {
    ...
    if (!f) return;
    fprintf(f, "#!/bin/bash\n");
    fprintf(f, "RESOLVE_LIBS=" INSTALL_DIR "/libs\n");
    fprintf(f, "if [ -n \"$LD_LIBRARY_PATH\" ]; then\n");
    fprintf(f, "  export LD_LIBRARY_PATH=\"$RESOLVE_LIBS:$LD_LIBRARY_PATH\"\n");
    fprintf(f, "else\n");
    fprintf(f, "  export LD_LIBRARY_PATH=\"$RESOLVE_LIBS\"\n");
    fprintf(f, "fi\n");
    fprintf(f, "mkdir -p \"$HOME/.local/share/DaVinci Resolve\" \"$HOME/.DaVinciResolve\" \"$HOME/.config/DaVinci Resolve\" \"$HOME/.cache/DaVinci Resolve\" 2>/dev/null\n");
    fprintf(f, "for bin in \"%s\" \"bin/Resolve\" \"bin/resolve\" \"Resolve\" \"resolve\" \"AppRun\"; do\n", bin_rel.c_str());
...
bool install_rooted(const std::string& appdir, const std::string& dep_cmd, std::string& error) {
    ...
    fprintf(sf, "find '" INSTALL_DIR "' -type f \\( -name '*.so*' -o -name 'resolve' -o -name 'AppRun' \\) -exec chmod 755 {} \\;\n\n");
    fprintf(sf, "mkdir -p '" INSTALL_DIR "'/configs '" INSTALL_DIR "'/logs '" INSTALL_DIR "'/.license '" INSTALL_DIR "'/.LUT '" INSTALL_DIR "'/Media '" INSTALL_DIR "'/Apple Immersive/Calibration\n");
